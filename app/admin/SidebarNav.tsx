...
export default function SidebarNav({ userName, role }: { userName: string; role: string }) {
  const pathname = usePathname()
  const [open, setOpen] = useState(false)

  const items = [
    ...navItems,
    ...(role === 'SUPER_ADMIN' ? [{ label: 'Usuarios', href: '/admin/usuarios', icon: '◈', exact: false }] : []),
  ]
  ...
  const sidebar = (
    <aside className="w-56 bg-white border-r border-gray-200 flex flex-col h-full">
      <div className="px-5 py-5 border-b border-gray-100 flex items-center justify-between">
        ...
        <button className="md:hidden text-gray-400 hover:text-gray-600 text-xl" onClick={() => setOpen(false)}>✕</button>
      </div>
